...
<div id="evcal_5" class="postbox evcal_admin_meta curve">	
	...
	<?php
		...
		foreach($qas as $section=>$questions){
			...
			foreach($questions as $question=>$answer){
				echo '<h5>'.$question.'</h5><p class="evodn">'.$answer.'</p>';
			}
		}
	...
	EVO()->elements->get_grid_content( array(
			...
			'box3' => array(
			    ...
			    'body'=> sprintf(
				    '<div class="evopadl30 evopadr30 evopadb20"><p>%s</p></div><div class=""> %s</div>',
				    esc_html__('Are you experiencing issues with EventON? Please look through our common questions/issues below and the solutions to them before contacting us.', 'text-domain'), wp_kses_post($faq_html) ),
			),
